...
const startMatch = "// -- HELPER: Interactive 3D Wrapper --";
...
if (startIndex === -1) {
    // Fallback if the comment was lost or changed
    startIndex = content.indexOf("export const ComputerSystemSimulator");
}
...
const Interactive3DView: React.FC<{ children: React.ReactNode, initialRot?: {x: number, y: number} }> = ({ children, initialRot = { x: 20, y: -30 } }) => {
   ...
   return (
      <div className="w-full h-full relative" style={{ perspective: 1500 }}>
         <div className="absolute top-4 w-full flex justify-center z-20 pointer-events-none">
            <span className="bg-slate-800/80 backdrop-blur px-3 py-1 rounded-full text-[10px] text-emerald-400 font-bold tracking-widest animate-pulse border border-emerald-500/30 shadow-xl">
...
// -- HELPER: Photorealistic CSS 3D Box --
const Box3D: React.FC<{ w: number, h: number, d: number, frontImg?: string, backImg?: string, frontColor?: string, sideColor?: string, backColor?: string, className?: string, children?: React.ReactNode }> = ({ w, h, d, frontImg, backImg, frontColor = '#334155', sideColor = '#1e293b', backColor = '#0f172a', className = '', children }) => {
   return (
      <div className={\`relative \${className}\`} style={{ width: w, height: h, transformStyle: 'preserve-3d' }}>
         {/* Front */}
         <div className="absolute inset-0 bg-cover bg-center shadow-inner" style={{ backgroundImage: frontImg ? \`url(\${frontImg})\` : 'none', backgroundColor: frontColor, transform: \`translateZ(\${d/2}px)\`, backfaceVisibility: 'hidden' }}>{children}</div>
         {/* Back */}
         <div className="absolute inset-0 bg-cover bg-center shadow-inner brightness-75" style={{ backgroundImage: backImg ? \`url(\${backImg})\` : (frontImg ? \`url(\${frontImg})\` : 'none'), backgroundColor: backColor, transform: \`rotateY(180deg) translateZ(\${d/2}px)\` }}></div>
         {/* Left */}
         <div className="absolute top-0 bottom-0 shadow-inner" style={{ width: d, backgroundColor: sideColor, transform: \`rotateY(-90deg) translateZ(\${d/2}px)\`, left: -d/2 }}></div>
         {/* Right */}
         <div className="absolute top-0 bottom-0 shadow-inner" style={{ width: d, backgroundColor: sideColor, transform: \`rotateY(90deg) translateZ(\${w - d/2}px)\`, left: -d/2 }}></div>
         {/* Top */}
         <div className="absolute left-0 right-0 shadow-inner" style={{ height: d, backgroundColor: sideColor, transform: \`rotateX(90deg) translateZ(\${d/2}px)\`, top: -d/2 }}></div>
         {/* Bottom */}
         <div className="absolute left-0 right-0 shadow-inner" style={{ height: d, backgroundColor: sideColor, transform: \`rotateX(-90deg) translateZ(\${h - d/2}px)\`, top: -d/2 }}></div>
      </div>
   )
}
...
const RealisticComponent: React.FC<{ type: string }> = ({ type }) => {
   switch(type) {
      case 'cpu': 
         return <Box3D w={70} h={70} d={4} frontImg="https://images.unsplash.com/photo-1591799264318-7e6ef8ddb7ea?auto=format&fit=crop&w=400&q=80" frontColor="#ccc" sideColor="#888" className="shadow-2xl" />;
      case 'ram': 
         return <Box3D w={180} h={25} d={4} frontImg="https://images.unsplash.com/photo-1562976540-1502c2145186?auto=format&fit=crop&w=400&q=80" frontColor="#10b981" sideColor="#064e3b" className="shadow-2xl" />;
      case 'motherboard': 
         return <Box3D w={300} h={300} d={6} frontImg="https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=800&q=80" frontColor="#1e293b" sideColor="#0f172a" className="shadow-2xl shadow-black/50" />;
      case 'gpu': 
         return <Box3D w={240} h={90} d={30} frontImg="https://images.unsplash.com/photo-1591488320449-011701bb6704?auto=format&fit=crop&w=400&q=80" frontColor="#111" sideColor="#222" className="shadow-2xl" />;
      case 'storage': 
         return <Box3D w={80} h={120} d={12} frontImg="https://images.unsplash.com/photo-1597872200969-2b65d56bd16b?auto=format&fit=crop&w=400&q=80" frontColor="#333" sideColor="#111" className="shadow-2xl" />;
      case 'keyboard': 
         return <Box3D w={300} h={100} d={10} frontImg="https://images.unsplash.com/photo-1595225476474-87563907a212?auto=format&fit=crop&w=400&q=80" frontColor="#222" sideColor="#111" className="shadow-2xl" />;
      case 'mouse': 
         return <Box3D w={60} h={100} d={20} frontImg="https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?auto=format&fit=crop&w=400&q=80" frontColor="#222" sideColor="#111" className="shadow-2xl rounded-[20px] overflow-hidden" />;
      case 'monitor': 
         return <Box3D w={280} h={160} d={10} frontImg="https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?auto=format&fit=crop&w=500&q=80" frontColor="#111" sideColor="#000" className="shadow-2xl" />;
      case 'printer': 
         return <Box3D w={200} h={140} d={120} frontImg="https://images.unsplash.com/photo-1612815154858-60aa4c59eae6?auto=format&fit=crop&w=400&q=80" frontColor="#e2e8f0" sideColor="#cbd5e1" backColor="#94a3b8" className="shadow-2xl" />;
      default: return null;
   }
}
   ...
   return (
      <div className="p-4 sm:p-6 bg-slate-900 rounded-3xl min-h-[500px] flex flex-col items-center">
         ...
         <p className="text-xs sm:text-sm font-medium text-slate-400 mb-8 text-center max-w-xl">
            Sistem Komputer terdiri dari 3 elemen utama. Pilih salah satu elemen untuk eksplorasi 3D, atau coba simulasi interaktif Merakit PC & Booting OS Windows 10 secara realistis.
         </p>
...
const HardwareExplorer: React.FC<{onBack: () => void}> = ({ onBack }) => {
   ...
   const components = [
      ...
      { id: 'storage', cat: 'Storage', name: 'Penyimpanan (SSD/HDD)', subtitle: 'Penyimpanan Permanen', desc: 'Tempat penyimpanan data, OS, dan aplikasi secara permanen.', color: 'from-purple-600 to-purple-800', icon: HardDrive },
   ];
      ...
      <div className="p-4 sm:p-6 bg-slate-900 rounded-3xl min-h-[600px] flex flex-col items-center relative overflow-hidden">
         ...
         {phase === 'assembly' && (
            <div className="flex flex-col lg:flex-row gap-6 w-full max-w-5xl h-full flex-1">
               <div className="w-full lg:w-[300px] bg-slate-800 p-4 rounded-3xl border border-slate-700 flex flex-col gap-2 overflow-y-auto max-h-[250px] lg:max-h-none">
                  ...
                  {['motherboard', 'cpu', 'ram', 'gpu', 'storage'].map(id => {
                     ...
                     return (
                        ...
                        >
                           <div className="w-8 h-8 shrink-0 flex items-center justify-center overflow-hidden rounded bg-black/20">
                              {/* Using tiny thumbnail view of the component for the list */}
                              <div className="scale-[0.25] origin-center"><RealisticComponent type={id} /></div>
                           </div>
               ...
               <div className="flex-1 bg-slate-950 rounded-3xl border-4 border-slate-800 flex items-center justify-center relative overflow-hidden shadow-2xl min-h-[400px]">
                  ...
                  {installed.includes('motherboard') && (
                     <div className="relative scale-75 sm:scale-100 transition-transform duration-500 ease-out" style={{ transformStyle: 'preserve-3d', transform: 'rotateX(55deg) rotateZ(15deg) scale(0.9)' }}>
                        <RealisticComponent type="motherboard" />
                        
                        {installed.includes('cpu') && (
                           <div className="absolute top-[80px] left-[110px] z-10 drop-shadow-2xl animate-in zoom-in duration-300" style={{ transform: 'translateZ(6px)', transformStyle: 'preserve-3d' }}>
                              <RealisticComponent type="cpu" />
                           </div>
                        )}
                        {installed.includes('ram') && (
                           <div className="absolute top-[60px] right-[40px] z-20 drop-shadow-2xl animate-in zoom-in duration-300 delay-100" style={{ transform: 'rotateZ(90deg) translateZ(8px)', transformStyle: 'preserve-3d' }}>
                              <RealisticComponent type="ram" />
                           </div>
                        )}
                        {installed.includes('gpu') && (
                           <div className="absolute bottom-[60px] left-[20px] z-30 drop-shadow-2xl animate-in zoom-in duration-300 delay-100" style={{ transform: 'translateZ(20px)', transformStyle: 'preserve-3d' }}>
                              <RealisticComponent type="gpu" />
                           </div>
                        )}
                        {installed.includes('storage') && (
                           <div className="absolute bottom-[20px] right-[20px] z-10 drop-shadow-2xl animate-in zoom-in duration-300 delay-100" style={{ transform: 'translateZ(6px)', transformStyle: 'preserve-3d' }}>
         ...
         {phase === 'pc-case' && (
            <div className="flex-1 w-full flex items-center justify-center animate-in zoom-in duration-500 min-h-[400px]">
               {/* PC Case 3D */}
               <Box3D w={180} h={320} d={300} frontColor="#111" sideColor="#222" backColor="#0a0a0a" className="shadow-[0_20px_50px_rgba(0,0,0,0.8)]" frontImg="https://images.unsplash.com/photo-1587202372775-e229f172b9d7?auto=format&fit=crop&w=400&q=80">
                  <div className="absolute inset-0 flex flex-col items-center py-10 bg-black/40 backdrop-blur-[2px]">
                     ...
                     <div className="absolute bottom-6 w-full px-8 flex justify-between">
                        <div className="w-2 h-2 rounded-full bg-blue-500/50 shadow-[0_0_5px_blue]"></div>
         ...
         {(phase === 'booting' || phase === 'windows' || phase === 'shutting-down') && (
            <div className="flex-1 w-full max-w-5xl flex items-center justify-center animate-in fade-in duration-1000 p-1 sm:p-4">
               {/* Monitor Frame */}
               <div className="w-full aspect-[16/10] sm:aspect-video bg-black border-[6px] sm:border-[16px] border-slate-900 rounded-lg sm:rounded-2xl shadow-2xl relative flex flex-col overflow-hidden">
                  ...
                  {phase === 'windows' && (
                     <div className="absolute inset-0 flex flex-col font-sans select-none overflow-hidden animate-in fade-in duration-1000 bg-black">
                        {/* Windows 10 Wallpaper Replica using Unsplash */}
                        ...
                        {/* Desktop Icons */}
                        <div className="p-2 sm:p-4 flex flex-col gap-4 sm:gap-6 z-0 h-full w-20 sm:w-24">
                        ...
                        <div className="absolute bottom-0 w-full h-[36px] sm:h-[40px] bg-[#101010]/95 backdrop-blur-xl border-t border-white/10 z-30 flex items-center justify-between px-1">
                           <div className="flex items-center gap-0 sm:gap-1 h-full">
                              ...
                              <button onClick={() => {setShowStart(!showStart); setShowPower(false);}} className={\`h-full px-2 sm:px-3 hover:bg-white/10 flex items-center justify-center transition-colors \${showStart ? 'bg-white/10' : ''}\`}>
                                 <div className="grid grid-cols-2 gap-[1px]" style={{ transform: 'perspective(20px) rotateY(15deg)' }}>
                                    <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-white"></div><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-white"></div>
                                    <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-white"></div><div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-white"></div>
                                 </div>
                              </button>
                              {/* Search */}
                              <div className="hidden sm:flex w-48 h-7 bg-white/5 border border-white/10 rounded px-2 items-center gap-2">
                                 <Search className="w-3 h-3 text-white/50" />
                                 <span className="text-white/40 text-[11px]">Type here to search</span>
                              </div>
                           </div>
                           <div className="flex items-center h-full text-white text-[10px] text-right px-2 hover:bg-white/10 cursor-default transition-colors">
                              <div className="flex gap-3 sm:gap-4 items-center mr-3">
                              ...
                              <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/90 ml-3" />
                           </div>
                        </div>
                     </div>
                  )}
...
`;
  ...
  console.log("Successfully replaced ComputerSystemSimulator with photorealistic 3D components and exact Windows 10 UI.");
} else {
